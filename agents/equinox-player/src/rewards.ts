...
export async function claimLobbyRewardsIfWinner(
  ...
  if (!winAnnounced.has(lobbyIdStr)) {
    ...
    console.log(
      `[equinox] WON lobby ${lobbyIdStr}! Prize pool credited: ${formatEther(prizePoolWei)} ETH (LobbyManager → playerBalance)`
    );
  }
  ...
  try {
    ...
    console.log(`[equinox] claimed ${formatEther(bal)} ETH to wallet ${account}`);
  } catch (e) {
    console.warn("[equinox] LobbyManager.withdraw failed", e);
  }
}
