...
- **`economyHints`** (trust these numbers over guessing):
  ...
  - **Winning needs balanced basics, not one huge pile:** each **`craftAlloy`** spends **`craftAlloyCost`** (10 food, 10 wood, 10 stone, 10 ore — see **`craftAlloyCost`**). To reach **`victoryGoodsThreshold`** alloy you must spend that cost **many times**, so your stockpiles should stay **roughly even** across the four basics. **Hoarding e.g. 900 food and 3 ore is a losing plan** — you cannot craft, and you waste turns. Aim to **diversify income** (more hexes + structures + biomes), then **rebalance** via trades or bank.
...
- **`rebalanceTradeDraft`** (inside **`economyHints`**, when not null) — precomputed barter (**`surplus`** → **`shortage`**) with concrete **`offer`** / **`request`** pouches. **Post `createTrade`** with those numbers (or close) and **open taker** (`0x0`) so peers can fill; do not ignore this every round while basics stay skewed.
...
3. **Rebalance to win:** If your **`resources`** show a huge gap (e.g. two basics very high, two very low), **do not** keep collecting the same hex — use **`createTrade`** / **`acceptTrade`** with **`peerAddresses`** to swap surplus for shortage. Prefer player trades when both sides gain; use **`bankTrade`** (4:1) when no peer deal works or when you **refuse to feed an opponent** — bank is a valid **unilateral** fix. **If any basic is ≥ 100**, dumping surplus via trade/bank/craft is **mandatory strategy**, not optional.
