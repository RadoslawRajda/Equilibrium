...
const idRel = (
  process.env.PLAYER_IDENTITY_PATH ||
  process.env.EQUINOX_IDENTITY_PATH ||
  "personas/equinox.md"
).trim();
const idPath = idRel.startsWith("/") ? idRel : join(root, idRel);
const system = [
  fs.existsSync(idPath) ? fs.readFileSync(idPath, "utf8") : "",
  ...
  .join("\n\n---\n\n");
if (!system.trim()) {
  console.error(`Missing identity (${idPath}) and/or skills/strategy.md`);
  process.exit(1);
}
...
await run("NO format (player-agent default)", {
  model,
  stream: false,
  messages: [
